...
double call_delta_mc(const int num_sims, const double S, 
                        const double K, const double r, 
                        const double v, const double T, const double delta_S) {

    // These values will be populated via the monte carlo call price function
    // They represent the incremented Sp (S+delta S ) , non−incremented S (S) and
    ...
    monte_carlo_call_price(num_sims, S, K, r, v, T, delta_S , 
                            price_Sp , price_S, price_Sm);
    return (price_Sp - price_S)/delta_S;
}

double call_gamma_mc(const int num_sims, const double S, 
                        const double K, const double r, 
                        const double v, const double T, const double delta_S) {
    double price_Sp = 0.0;
    double price_S = 0.0;
